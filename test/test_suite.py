...
# https://github.com/sqlalchemy/sqlalchemy/blob/main/test/dialect/test_sqlite.py
...
class DialectTest(
    ...
    def test_pool_class(self):
        e = create_engine("sqlite+libsql://")
        assert e.pool.__class__ is pool.SingletonThreadPool

        e = create_engine("sqlite+libsql:///:memory:")
        assert e.pool.__class__ is pool.SingletonThreadPool

        e = create_engine("sqlite+libsql:///file:foo.db?mode=memory&uri=true")
        assert e.pool.__class__ is pool.SingletonThreadPool

        e = create_engine("sqlite+libsql:///foo.db")
        # changed as of 2.0 #7490
        assert e.pool.__class__ is pool.QueuePool

        e = create_engine("sqlite+libsql://localhost:8080")
        assert e.pool.__class__ is pool.SingletonThreadPool

    @combinations(
        (
            "sqlite+libsql:///foo.db",  # file path is absolute
            ([os.path.abspath("foo.db")], {"check_same_thread": False}),
        ),
        (
            "sqlite+libsql:////abs/path/to/foo.db",
            (
                [os.path.abspath("/abs/path/to/foo.db")],
                {"check_same_thread": False},
            ),
        ),
        ("sqlite+libsql://", ([":memory:"], {"check_same_thread": True})),
        (
            "sqlite+libsql:///?check_same_thread=true",
            ([":memory:"], {"check_same_thread": True}),
        ),
        (
            "sqlite+libsql:///file:path/to/database?"
        ...
        (
            "sqlite+libsql:///file:path/to/database?" "mode=ro&uri=true",
            (
        ...
        (
            "sqlite+libsql:///file:path/to/database?uri=true",
            (
        ...
        (
            "sqlite+libsql://localhost:8080?authToken=JWT_HERE&secure=true",
            (
                ["wss://localhost:8080?authToken=JWT_HERE"],
                {"uri": True, "check_same_thread": True},
            ),
        ),
        (
            "sqlite+libsql://localhost:8080?authToken=JWT_HERE",
            (
                ["ws://localhost:8080?authToken=JWT_HERE"],
                {"uri": True, "check_same_thread": True},
            ),
        ),
    )
...
class InsertTest(fixtures.TestBase, AssertsExecutionResults):
    ...
    def teardown_test(cls):
        with testing.db.begin() as conn:
            for name in ["a", "b", "c", "d", "e", "f", "tbl"]:
                conn.exec_driver_sql("drop table if exists %s" % name)
